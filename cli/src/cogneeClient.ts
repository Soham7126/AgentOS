...
export function recall(query: string, filters?: Record<string, unknown>, top_k = 15) {
  return post<{ results: RecallResult[] }>("/recall", { query, filters, top_k });
}

export function improve(session_id?: string) {
  return post<{ status: string }>("/improve", { session_id });
}
